...
public class DivisionRepoJdbcTemplate implements DivisionRepo{
	...
	private static final String SELECT_SQL = """
			select dv.id,dv.name, count(ds.id) from DIVISION dv 
	...
	public List<DivisionDto> search(String name) {
		...
		if(StringUtils.hasLength(name)) {
			...
			params.add(name.toLowerCase().concat("%s"));
		}
	...
	public Optional<DivisionDto> findById(int id) {
		...
		return template.query("%s where id = ? group by dv.id, dv.name".formatted(SELECT_SQL), rowMapper, id)
			.stream().findAny();
